# 0044 - 只用一次的泛型参数改写为 opaque parameter（`some`）
...
- **2026-09-26 In Progress → Implemented，落地编号 0044**：代码已于 2026-09-18 随 `7d429938` 合入 `next`，当时状态停在 In Progress、没有取号；0.20.0 发版收尾时按合入顺序补取。按头部的判断不需要配套文档（纯签名写法的统一）；没有新的项目术语。
