...
- **SwiftThunkAnalysis** — reads an availability-conditional opaque type (a kind-9 accessor-function symbolic reference) by symbolically evaluating the thunk instead of running it. Capstone, ARM64 only. Also home of `ObjCAncestorOverrides` (`ObjCOverride/`): the `override` of an ObjC-inherited member is invisible to Swift metadata (the compiler emits a NEW vtable entry for it, and an `@objc @implementation` class has no vtable), so it is recovered from the class's own ObjC method table against its ancestors' selectors and joined to the Swift member by the `To` thunk symbol at the IMP or — OS frameworks strip those — by decoding the anonymous thunk for the implementation it calls or materializes, guarded by the member's name being the importer's spelling of the selector; a name-only third tier exists behind `infersOverridesFromSelectorNames` (default off). `isOverride` / `isClassMember` on the member definitions OR the fact in, so `override class func` prints where `static` did. [Modules/SwiftThunkAnalysis.md](Documentations/Internal/Modules/SwiftThunkAnalysis.md).
...
- **MachODependencies** — the one dependency-resolution implementation every feature shares (`DependencyClosure`, the in-process and file locators, `DependencySearchPath`). [Modules/MachODependencies.md](Documentations/Internal/Modules/MachODependencies.md).
...
# thunk reader, the static layout engine and the indexer's cross-image facts (a stored
# field whose type is a property wrapper from another image) at the images a standalone
# binary links — a Mach-O file, a dyld cache file, or a directory used as a system root.
# Without it, paths are inferred from where the binary sits and the running system's
# cache is used.
