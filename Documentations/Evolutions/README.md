...
| [draft](draft-missing-abi-structures.md) | 补齐五组缺失的 ABI 结构：async / coro function pointer 记录、capture descriptor（`__swift5_capture`）、generic metadata pattern 家族、accessible function record（`__swift5_acfuncs`）、function type metadata 尾随对象 | Accepted |
