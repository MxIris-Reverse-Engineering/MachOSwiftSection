...
- `ABISnapshot` / `ABISnapshotDocument` - Frozen `Codable` projection + the versioned persistence envelope (`formatVersion` — bump on any key-scheme change, decode fails typed on mismatch; currently 3: v2 folded `indirect` into enum-case keys, v3 split extension containers per conformance — plus `ABIProvenance`: label / binary path / generator version / date). `ABIJSON` is the one JSON dialect (ISO-8601, sorted keys) so baselines are byte-stable
...
- See [Documentations/Internal/ABIDiffDesignAndLimitations.md](Documentations/Internal/ABIDiffDesignAndLimitations.md), [Documentations/Internal/ABIEvolutionDesign.md](Documentations/Internal/ABIEvolutionDesign.md), and [Documentations/Internal/PerConformanceAttribution.md](Documentations/Internal/PerConformanceAttribution.md)
