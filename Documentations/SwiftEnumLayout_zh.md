...
`swift-section dump --emit-enum-layout`（以及库内渲染器）输出的注释由 token 模板 `Transformer.SwiftEnumLayout`（`OutputTransformer` 模块）驱动。三层模板对应注释结构——类型级策略行、逐 case 块、逐固定字节行——用 `${token}` 占位（和 RuntimeViewerCore 的 transformer UI 同名）。内置五种预设，CLI 通过 `--enum-layout-style` 选择：
