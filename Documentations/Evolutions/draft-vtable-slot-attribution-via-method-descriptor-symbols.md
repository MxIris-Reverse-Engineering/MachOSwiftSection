...
| 2026-09-07 | 上面 2026-09-06「墓碑槽」那条决定的**因果前提被推翻**，待修复批次重新裁定 | 该决定假定「实现指针为 null ⇒ 成员被删除」。fixture 证据不支持：`public final class TestsObjects {}`（隐式 `init()` 明确存在）等 8 个类的 `__allocating_init` 全部被打上「deleted method」，fixture 里所有非 async 的 `__allocating_init` 无一幸免；且 SwiftUICore 上 33% 的槽都是「被删除的方法」这一比例本身不可信。「实现指针为 null」是 `next` 上就有的老观察（那时输出 `Symbol not found`），新加的是对它的因果解释，而这个解释站不住。注释应只陈述可观察事实；若要保留墓碑断言，须实际验证 class metadata 对应的 word 是否 bind 到 `swift_deletedMethodError`——那是可查的，`GraphHostVTableAttributionTests` 的文档注释已如此声称但代码并未验 |
