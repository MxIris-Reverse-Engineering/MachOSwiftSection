# 0011 - opaque 返回类型的 primary associated type 归属：anchor 协议裁决 + 协议事实解析链
...
| 2026-08-24 | 编号 0005 → 0006（rebase 整合进 main） | push 时发现远端 main 已合并 release/0.16.0 线，编号 0005 被该线的《降级上报统一走事件》占用；本案 rebase 到新 main 之上并整体改号 0006（文件名、代码注释、fixture 注释、全部文档互链）。同批补录：项目 `Glossary.md`（0.16.0 线建立）登记本案三术语，ProjectEvolutionLog 节号让位为 42。 |
| 2026-08-24 | 编号 0006 → 0011（并入 next） | 按用户要求把 main 直落的本案以 PR 并入 `next`；next 线的提案已占用 0006–0010（final 关键字、extension 容器去重、interface 头部、TypeIndexing 重启、补充类型映射），本案第二次整体改号 0011，ProjectEvolutionLog 节号再让位为 46。next 也重生成过全部 fixture baseline 且渲染行为有变（0006/0008），合并状态上重建 fixture、重跑 regen-baselines 并按测试结果重录受影响 snapshot。 |
