...
extension RelativeIndirectPointerProtocol {
    ...
    func resolveIndirect<T: Resolvable>(from fileOffset: Int, in machOFile: MachOFile) throws -> T {
        return try resolveIndirectType(from: fileOffset, in: machOFile).resolveAny(in: machOFile)
    }

    public func resolveIndirectType(from fileOffset: Int, in machOFile: MachOFile) throws -> IndirectType {
