...
- **回归对比**：`/tmp/macho-refactor-verify/old`（main 重构前）vs 修复后重跑，SwiftUI / SwiftUICore
  ...
  复现 harness：`Tests/SwiftInterfaceTests/RefactorVerifyInterfaceTests.swift`（临时验证文件，options 由
  `$RV_OPTS` 控制，默认全开），`USING_LOCAL_DEPENDENCIES=1 RV_OUT=<dir> swift test --filter RefactorVerifyInterfaceTests`。
