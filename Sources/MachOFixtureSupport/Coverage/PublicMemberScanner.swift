...
///   filtered.
/// - `init(layout:offset:)`, hand-written or macro-generated alike
...
///
/// Added back:
/// - `layout` / `offset` on a struct carrying `@LocatableLayoutWrapping`. This
///   scanner reads source, not macro expansions, and those two properties were
///   spelled out by hand in every wrapper until the macro absorbed them — so
///   they are re-added here to keep the coverage contract (and the 96 baseline
///   files registering them) exactly as it was.
///
...
private final class PublicMemberVisitor: SyntaxVisitor {
    ...
    override func visit(_ node: StructDeclSyntax) -> SyntaxVisitorContinueKind {
        typeStack.append(stripBackticks(node.name.text))
        spiStack.append(hasSPI(attributes: node.attributes))
        collectLocatableLayoutWrappingMembers(of: node)
        return .visitChildren
    }
    ...
    /// `@LocatableLayoutWrapping` generates `layout`, `offset` and
    /// `init(layout:offset:)` at the host's own access level. Only the two
    /// properties are recorded — the initializer is skipped for macro-generated
    /// and hand-written wrappers alike (`isMemberwiseSynthesizedInit`).
    ///
    /// Called after the host has been pushed onto the scopes, so an enclosing
    /// `@_spi` still disqualifies it.
    private func collectLocatableLayoutWrappingMembers(of node: StructDeclSyntax) {
        guard hasAttribute(named: "LocatableLayoutWrapping", attributes: node.attributes) else { return }
        guard isPublicLike(node.modifiers, attributes: node.attributes) else { return }
        guard let typeName = currentTypeName(), !shouldSkip(typeName: typeName) else { return }
        collected.append(MethodKey(typeName: typeName, memberName: "layout"))
        collected.append(MethodKey(typeName: typeName, memberName: "offset"))
    }

    private func hasAttribute(named name: String, attributes: AttributeListSyntax) -> Bool {
        for attribute in attributes {
            if let attributeSyntax = attribute.as(AttributeSyntax.self),
               attributeSyntax.attributeName.trimmedDescription == name {
                return true
            }
        }
        return false
    ...
    private func isMemberwiseSynthesizedInit(_ node: InitializerDeclSyntax) -> Bool {
        // The LocatableLayoutWrapper initializer, whether hand-written or
        // generated by @LocatableLayoutWrapping. Exercised through every
        // wrapper's own reads, never as a standalone member.
