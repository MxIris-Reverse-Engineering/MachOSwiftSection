...
class SwiftInterfaceBuilderMachOFileTests: MachOFileTests {
    ...
    @Test func buildFile() async throws {
        ...
        builder.setDependencyPaths([.usesSystemDyldSharedCache])
        try await builder.prepare()
        try builder.build().string.write(to: .desktopDirectory.appending(path: "\(machO.imagePath.lastPathComponent)-Dump-iOS.swiftinterface"), atomically: true, encoding: .utf8)
    }
}
