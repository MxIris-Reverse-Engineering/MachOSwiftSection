...
| 2026-09-19 | 追记：`declarationNodeKinds` 的字面量提到文件级 `private let`，计算属性只返回它 | 用户问「`static var` 里放字面量会不会被优化成 `static let`」，用 Swift 6.4 编样本看优化后 SIL：数组字面量会外提成静态数据、标量会内联，但 `Set` 字面量每次访问都重建（构造依赖进程随机 hash seed，折不掉），每次一次堆分配加哈希；实测计算型 `static var` 约 70 ns / 次、文件级常量约 17 ns / 次、非泛型 `static let` 约 14 ns / 次、`switch` 约 1 ns / 次。解包链每次 printRoot 只访问几次，量级无关紧要，但改成文件级常量零成本且保留 `Set` 与错误里的 `expected:`；`switch` 更快但候选集合得另找表达方式，不值 |
