...
First pre-release on the road to `0.12.0`. Headline work since `0.11.0`:
...
The upshot: `swift-section dump` / `interface` on a file now emit real field offsets without loading the process. See [Documentations/Internal/FieldLayoutRendererReaderSpecialization.md](Documentations/Internal/FieldLayoutRendererReaderSpecialization.md).

### `SwiftDump → leaf`: `SwiftDeclarationRendering` extraction

Field-metadata rendering used to live in `SwiftDump`. It now lives in a new `SwiftDeclarationRendering` module shared by `SwiftDump` and `SwiftInterface`, with `SwiftDump` reduced to a leaf in the dependency graph. The declaration model is now `Codable` end-to-end. See [Documentations/Internal/SwiftModularizationMigration.md](Documentations/Internal/SwiftModularizationMigration.md).
...
This is a pre-release on the road to `0.12.0`. The public-facing CLI surface (`swift-section diff`, the offline `--emit-*` rendering paths) is expected to remain compatible through `0.12.0`, but internal `SwiftLayout` and `SwiftDeclarationRendering` APIs may still shift.
