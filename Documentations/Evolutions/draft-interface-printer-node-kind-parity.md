...
| 2026-09-15 | 使用位置的 `each` 改读 `packExpansion` 的 count type（child 1） | 第一版用启发式「pattern 只含一个 distinct 参数时那个就是 pack」，`repeat (T, each U)` 因两个参数而整个放弃，并在注释里断言「mangling 说不出哪个是 pack」——**这句是错的**。用户追问后查证：`packExpansion` 有两个 children，child 1 就是驱动展开的那个 pack（mangling 依次写 pattern、count type、`Qp`）。换掉启发式后严格更强、代码更简单。仍未覆盖的只有一次展开多个 pack（`repeat (each A, each B)`），count type 只点名一个 |
