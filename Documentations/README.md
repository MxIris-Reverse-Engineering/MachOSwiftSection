...
| [LeafMigrationRegressionAudit.md](Internal/LeafMigrationRegressionAudit.md) | 对 `aa233bc` leaf 迁移线的全面回归审计：三路逐行比对方法、仍存活的 7 项问题清单（多 payload 枚举容错丢失、深度截断诊断静默 + 测试钉死常量、Void payload case 两路不一致等，待逐项决定）、已修复的历史断裂记录（metadata 注释全丢 / SIGBUS / 绑定渲染回归）与已核对干净的面。 |
