...
├── GenericArgumentEnvironment.swift # 阶段 5/6：bound-generic 实例化的 (depth,index)→实参 Node 映射 + 自定义 top-down 语法替换；实参可为类型 / value（integer）/ pack；tuple/pack 内的具体 pack expansion 就地展开（含单元素坍缩与转发扁平化）
...
├── ExistentialLayoutBridge.swift   # existential 容器 + existential metatype + 受约束/扩展 existential（ExistentialTypeInfoBuilder 移植）
...
├── ImageReference.swift            # 单镜像：类型限定名 → descriptor 索引 + 协议 class 约束索引 + ObjC 类 instanceSize 索引 + assocty witness 索引 + builtin 索引（缺段容忍）
├── ImageUniverse.swift             # singleImage / dependencyClosure；类型/协议/ObjC-类/assocty-witness 四个解析 seam；依赖惰性索引
...
| 形态 | 原因 | 归属阶段 |
|---|---|---|
| 泛型类型**自身**作顶层且**未提供实参**（裸 `T` / 裸 `each T` / 裸 `let N`） | 无具体实参时字段里的参数依赖实例化；**提供具体实参后已可算**（含 value/pack 实参，见「后续工作」） | 裸泛型不可静态定（本质如此） |
