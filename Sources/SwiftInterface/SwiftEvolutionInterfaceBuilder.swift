...
///
/// The class is deliberately **non-generic**: each version is erased at
/// construction, so the homogeneous-array initializer (all versions the same
/// reader type — the CLI's N `MachOFile`s) and the parameter-pack initializer
/// (heterogeneous reader types, gated on the Swift 5.9 runtime that parameter
/// packs require) both produce the same type.
public final class SwiftEvolutionInterfaceBuilder: Sendable {
    ...
    /// Homogeneous construction: N versions of the same reader type.
    /// Available on every platform this package supports.
    ///
    ...
    /// (e.g. a standalone `MachOFile` next to a dyld-cache image). Parameter
    /// packs need the Swift 5.9 runtime (SE-0393), hence the availability
    /// floor; on older systems use the homogeneous initializer.
...
/// Usage-contract failures of `SwiftEvolutionInterfaceBuilder`. (Input-shape
