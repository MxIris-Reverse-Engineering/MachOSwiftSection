...
- `ClassBoundGenericParameterAnalysis` - Phase-9 **unspecialized** class-bound parameter resolution: a generic parameter constrained to a class layout (`Element: AnyObject`), a superclass (`Element: SomeClass`, a `baseClass` requirement), or a class-bound protocol (`Element: SomeClassBoundProtocol`, Swift or imported/`@objc` ObjC) is necessarily a single object reference, so a field typed by it — **and every field after it** — lays out exactly even when the type is dumped with no generic arguments. Reads the descriptor's cumulative canonical requirement signature (`genericContext.requirements`; requirement subjects carry absolute `(depth, index)`, matching field records, so nested contexts need no extra bookkeeping), demangles each requirement's bare-parameter subject (a constraint on a dependent member like `Element.Index` is ignored — it does not pin `Element`), and classifies class-boundness from the requirement kind + resolved content (`layout(.class)` / `baseClass` / a class-bound `protocol`; a cross-image protocol symbol is recovered by name through the universe's `resolveProtocolClassConstraint` + `isObjCProtocolDeclared` seams). The result seeds `GenericArgumentEnvironment.classBoundParameterKeys` via `augmentedWithClassBoundParameterKeys`, applied at every field-reading entry point (`StaticLayoutCalculator.fieldLayout(ofStruct:/ofClass:)` and the resolver's `computeStructLayout` / `computeClassLayout` choke points, so a generic superclass `class Sub<Element: AnyObject>: Base<Element>` and ObjC-ancestor subclasses benefit too). An unsubstituted `dependentGenericParamType` whose key is in the set rewrites — through the same top-down substitution, so it reaches inside optionals/tuples/`Array<Element>`/function types — to a placeholder `.class` node the resolver lays out as `.pointerSized` (`swift_getHeapObjectExtraInhabitantCount` = 0x1000, matching a concrete class field exactly; the placeholder's name is inert — a class node is never recursed into). A genuine substitution always wins over the class-bound fallback (a concrete instantiation of a class-bound parameter uses its actual argument), and a class-bound generic multi-payload enum still takes the tagged branch (the augmented environment is non-empty, gating the spare-bits path off as for any instantiation). `sameType`-to-a-concrete-class is left to the specialization path, not treated as a representation constraint here
