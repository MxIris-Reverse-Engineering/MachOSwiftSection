...
| [Evolutions/0003-symbol-row-bucket-flattening.md](Evolutions/0003-symbol-row-bucket-flattening.md) | **提案 0003（In Review）**：`SymbolIndexStore` `[UInt32]` 行号桶扁平化——45 万个小数组（38.8 MiB，绝大多数单元素、各付 ~48 B 堆分配）换「单元素内联、多元素落堆」的 `SymbolRowBucket`，覆盖 `symbolRowsByOffset` 与三族 `MemberSymbolRows` 叶子桶；预估省 15–25 MiB，纯内部零 API 变化。0001「非目标」点名候选的正式立项。 |
