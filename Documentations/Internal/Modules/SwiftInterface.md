...
**`SwiftInterfaceBuilderExtraDataProvider`**：外部数据源的注入缝，`TypeNameResolvable` + 一个默认为空的 `setup()`。`addExtraDataProvider(_:)` 同时把 provider 注册为 printer 的 type-name resolver，所以它一头挂在 builder 的 prepare 生命周期上，一头挂在打印热路径上。两个已知实现：`TypeIndexing.SwiftInterfaceBuilderTypeNameProvider`（`__C` 模块归属，跨模块）和本模块的 `SwiftInterfaceBuilderOpaqueTypeProvider`（见子系统 2）。
