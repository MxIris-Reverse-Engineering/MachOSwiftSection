...
| 2026-09-11 | 删掉本批所有 per-field 的 `…Offset` 属性，改用 `LocatableLayoutWrapper.resolvedDirectOffset(from:)` | 用户指出那些属性没意义：每一个都是同样三行，key path 已经在调用点写出了字段名。顺带把提案 0018 的五个既有 `implementationOffset`（公开 API，名字保留）改成一行，并把 `TypeGenericContextDescriptorHeader` / `SingletonMetadataInitialization` 两处 Layout 字段从裸 `RelativeOffset` 改成 `RelativeDirectRawPointer`，否则它们用不上这个 helper、会成为唯一剩下的手写特例 |
| 2026-09-11 | 同样删掉纯转发属性（`classFlags` / `expectedContextSize` / `numberOf*` 这类），只保留真正有解码或推导的访问器 | 用户指出 `LayoutWrapper` 是 `@dynamicMemberLookup`，`layout` 的字段本来就能直接读；只做 `Int(...)` 加宽的那些也一并删掉，按 `FieldDescriptor` / `TupleTypeMetadata` 的既有做法在使用点转换——同名不同类型的属性会遮蔽 dynamic member，是个陷阱。被删属性的文档事实搬到对应 Layout 字段上 |
