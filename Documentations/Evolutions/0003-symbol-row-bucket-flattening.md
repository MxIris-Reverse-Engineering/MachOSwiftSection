...
**这个提案解决什么问题**：提案 0001（符号名 offset 化）落地后复测，堆里还剩 45 万个小数组、合计 38.8 MiB。它们是符号索引里的「桶」——记录一个地址或一个成员对应哪几行符号的 `[UInt32]` 数组。绝大多数桶其实只装一个元素，却每个都要付一次约 48 字节的堆分配。

**怎么解决**：新增一个二十来行的枚举类型 `SymbolRowBucket`：桶里只有一个元素时，这个元素直接内联存在字典槽里，零堆分配；出现第二个元素时才分配真正的数组。实测 87.6% 的桶是单元素，正中这个表示的甜点。

**结果**：45 万个碎数组坍缩成 5 个字典，38.8 → 7.2 MiB（超出预估的 15–25 MiB 带）。零公开 API 变化，查询输出逐字节不变。与同批独立实施的提案 0002（声明模型瘦身，另一个模块）合计，把 RuntimeViewer 五镜像稳态从 322 MB 推到 262 MB。

**读正文的导航**：被否的更激进方案（CSR 全量平铺——省得更多但要改的面大三倍）见「替代方案考量」；每步验证与实测数字见「落地步骤」的勾选项和「决策日志」。
