...
- 类型改名反映新范围（这些类型只在 `next` 上、未随任何版本发布）：`ObjCAncestorOverride` → `ObjCMember`（`selector` / `isClassMethod` / `evidence` / `overriddenAncestorClassName: String?` / `hasExplicitSelector: Bool`），`ObjCAncestorOverrideTable` → `ObjCMemberTable`，构建器 `ObjCAncestorOverrides` → `ObjCMembers`（`infersOverridesFromSelectorNames` 开关随之搬家，2026-09-22 后整个删除），`ObjCAncestorOverrideApplication` → `ObjCMemberApplication`，定义上的 `objcAncestorOverride` → `objcMember`。RuntimeViewer 若已引用旧名，留 deprecated typealias 一个版本。
- `isOverride` / `isClassMember` 改读 `objcMember?.overriddenAncestorClassName != nil`，语义不变。**2026-09-22 修订**：改读 `objcMember?.isJoinedOverride`，只认前两档；第三档的裁决交给 `ResolvedObjCMemberFacts`（见决策日志同日第二条）。
